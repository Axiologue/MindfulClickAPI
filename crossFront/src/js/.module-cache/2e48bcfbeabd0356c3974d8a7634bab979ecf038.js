...
var CrossList = React.createClass ({displayName: "CrossList",
  ...
  render: function() {
    ...
    var articleNodes = this.state.data.map(function (article) {
      return (
        React.createElement(CrossWrapper, {url: article.url, title: article.title, notes: article.notes})
      );
    });
...
React.render(
  React.createElement(CrossList, {url: "http://localhost:8000/cross/articles"}),
