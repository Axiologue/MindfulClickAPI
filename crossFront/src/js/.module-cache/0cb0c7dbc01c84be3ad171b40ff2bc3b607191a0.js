...
var ArticleWrapper = React.createClass ({displayName: "ArticleWrapper",
  ...
  render: function() {
    ...
    if (!this.state.add_analysis && !this.state.edit) {
      var normalView = (
          React.createElement("div", {className: "article listitem col-xs-12"}, 
            ...
            React.createElement(WarningModal, {message: '<p>Are you sure you want to delete the article "<em>' + this.state.title + '</em>"?</p><p>This will not only remove the article, but any associated analysis that has done on it.  This cannot be undone.</p>', 
              id: this.props.id, 
...
var CrossWrapper = React.createClass ({displayName: "CrossWrapper",
  render: function() {
    var crossData = this.props.data.map(function (cross) {
      ...
      var scoreClass = cross.score > 0 ? 'positive' : 'negative';
      var dlClass = 'dl-horizontal crossItem ' + scoreClass;

      return (
        React.createElement("li", null, 
          React.createElement("dl", {className: dlClass}, 
            React.createElement("dt", null, cross.company, " ", cross.product), 
            React.createElement("dd", null, cross.subcategory, ": ", React.createElement("span", {className: scoreClass}, cross.score)), 
            React.createElement("dd", null, React.createElement("span", {className: "text-muted"}, cross.notes))
          )
        )
      );
    });
